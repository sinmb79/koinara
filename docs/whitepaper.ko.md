...
이 뒤에 이어지는 appendix는 같은 정식 백서의 일부이며, 별도의 독립 문서가 아니다. 본문이 프로토콜의 논지와 최소 메커니즘을 제시한다면, appendix는 배포 프로파일, 이식성 메모, 컨트랙트 매핑, 파라미터 참조를 모아두는 annex 역할을 맡는다.
...
이 문서에서 Worldland는 프로토콜 의존성이 아니라 reference deployment profile이다.
...
이 부록은 v1에서 비규범적이다. 완성된 구현이 아니라 이식 가능성을 보여준다.
...
이 부록 역시 v1에서 비규범적이다. 구현 기능이라기보다 가능한 anchoring 방향을 설명한다.
...
주요 아키텍처 도식은 Section 6에 실려 있다. 이 appendix는 그 계층을 v1 참조 컨트랙트에 대응시킨다.
...
이 매핑은 본문을 읽기 쉬운 형태로 유지하면서도, 추상적인 프로토콜 계층을 구체적인 참조 코드베이스와 연결해 준다.
