...
import { Btn, Card, Divider, JobTypeBadge, JobStateBadge, Spinner, TxLink, AddrLink } from '../components/ui.jsx'
...
export default function JobDetail() {
  ...
  useEffect(() => {
    const found = jobs.find((entry) => entry.id === jobId)
  ...
  const fetchDetails = async () => {
    const [nextResponses, nextPoi] = await Promise.all([getResponses(jobId), getPOI(jobId)])
    setResponses(nextResponses)
    if (nextPoi) setPoi(nextPoi)
  }
  ...
  const handleSubmitResponse = async () => {
    ...
    try {
      ...
      toast.error(error.reason ?? error.message ?? t('jd_err_tx'))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      toast.error(error.reason ?? error.message ?? t('jd_err_claim'))
    } finally {
      setClaiming(false)
    }
  }
  ...
  if (!job) {
    return <div style={{ display:'flex', justifyContent:'center', padding:'160px 0' }}><Spinner size={32} /></div>
  }
  ...
  const myResponse = responses.find((entry) => entry.provider?.toLowerCase() === address?.toLowerCase())

  return (
    <div style={{ maxWidth:900, margin:'0 auto', padding:'100px clamp(20px, 5vw, 40px) 80px' }}>
      <Link to="/" style={{ fontFamily:'Share Tech Mono', fontSize:'0.78rem', color:'#6b7a99', display:'inline-flex', alignItems:'center', gap:6, marginBottom:28 }}>
        {t('jd_back')}
      </Link>
        ...
        <div>
          ...
          {job.state === 0 && !isPastDeadline && (
            <Card>
              ...
              {myResponse ? (
                ...
                <>
                  <textarea rows={6} value={responseText} onChange={(event) => setResponseText(event.target.value)} placeholder={t('jd_response_ph')} style={{ marginBottom:12 }} />
          ...
          {responses.length > 0 && (
            <div style={{ marginTop:24 }}>
              <h3 style={{ fontFamily:"'Syne',sans-serif", fontSize:'1rem', marginBottom:16 }}>{t('jd_responses_list', responses.length)}</h3>
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                {responses.map((response, index) => (
                  <Card key={`${response.provider}-${index}`}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8, gap:12, flexWrap:'wrap' }}>
                      <AddrLink addr={response.provider} />
                      <span style={{ fontFamily:'Share Tech Mono', fontSize:'0.7rem', color: response.accepted ? '#00ffb4' : '#6b7a99' }}>
                        {response.accepted ? t('jd_accepted') : `Votes ${response.approvalCount}/${quorum}`}
                      </span>
                    </div>
                    {response.text && (
                      <p
                    ...
                    <div style={{ fontFamily:'Share Tech Mono', fontSize:'0.68rem', color:'#3a4560', marginTop:8 }}>
                      hash: {response.responseHash?.slice(0,20)}...
                    </div>
                  </Card>
                ))}
              </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          <Card>
            ...
            {[
              ...
              [t('jd_weight'), `x${weight} KOIN`],
              [t('jd_quorum'), t('jd_quorum_unit', quorum)],
              [t('jd_premium'), Number(premium) > 0 ? `${premium} WLC` : t('jd_premium_none')],
            ].map(([key, value]) => (
              <div key={key} style={{ display:'flex', justifyContent:'space-between', padding:'8px 0', borderBottom:'1px solid rgba(0,255,180,.06)', gap:12 }}>
                <span style={{ fontSize:'0.78rem', color:'#3a4560', textTransform:'uppercase', letterSpacing:'0.05em' }}>{key}</span>
                <span style={{ fontFamily:'Share Tech Mono', fontSize:'0.82rem', color:'#e8f0ff', textAlign:'right' }}>{value}</span>
              </div>
            ))}
            <div style={{ display:'flex', justifyContent:'space-between', padding:'8px 0', gap:12 }}>
              ...
              <span style={{ fontFamily:'Share Tech Mono', fontSize:'0.78rem', color: isPastDeadline ? '#ff4466' : '#ffaa00', textAlign:'right' }}>
                {isPastDeadline ? t('jd_expired') : formatDistanceToNow(deadline, { addSuffix:true, locale })}
              </span>
            </div>
