...
import { Btn, Card, Tag, Badge, Field } from '../components/ui.jsx'
...
const MOCK_PROVIDERS = [
  ...
  { name:'HybridStack-a', type:2, addr:'0xdef0...6789', poi:892, rate:'97.4%', jobs:621 },
  { name:'ResearchNet-X', type:2, addr:'0x2468...ace0', poi:567, rate:'96.9%', jobs:401 },
]

const COLORS = ['green', 'blue', 'purple']
const LABELS = ['AI', 'VR', 'HY']
...
export default function Providers() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      toast.error(error.reason ?? error.message ?? t('pv_err_tx'))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div style={{ maxWidth:1100, margin:'0 auto', padding:'100px clamp(20px, 5vw, 40px) 80px' }}>
      <div style={{ marginBottom:48 }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', flexWrap:'wrap', gap:20 }}>
          ...
          <Btn variant="primary" onClick={() => setShowForm((open) => !open)}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))', gap:10, marginBottom:20 }}>
            {nodeTypeOptions.map((option) => (
              <div
                ...
                style={{
                  border:`1px solid ${nodeType === option.value ? '#00ffb4' : 'rgba(0,255,180,.1)'}`,
                  background: nodeType === option.value ? 'rgba(0,255,180,.06)' : 'transparent',
                  borderRadius:10,
                  padding:16,
                  cursor:'pointer',
                  transition:'all .2s',
              ...
              >
                ...
                <p style={{ fontSize:'0.8rem', color:'#6b7a99', marginTop:8 }}>{option.desc}</p>
              </div>
            ))}
          </div>

          <Field label={t('pv_field_endpoint')}>
            <input value={endpoint} onChange={(event) => setEndpoint(event.target.value)} placeholder="https://your-agent-endpoint.com" />
          </Field>

          <div style={{ marginTop:16 }}>
            <Btn variant="primary" loading={loading} onClick={handleRegister}>
              {t('pv_btn_onchain')}
            </Btn>
          </div>
        </Card>
        ...
        {MOCK_PROVIDERS.map((provider) => (
          <Card key={provider.name} hover>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14, gap:10 }}>
              ...
              <Badge color={COLORS[provider.type]}>{nodeTypeOptions[provider.type]?.label}</Badge>
            </div>

            <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:600, fontSize:'0.95rem', marginBottom:4 }}>{provider.name}</div>
            <div style={{ fontFamily:'Share Tech Mono', fontSize:'0.68rem', color:'#3a4560', marginBottom:14 }}>{provider.addr}</div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, paddingTop:14, borderTop:'1px solid rgba(0,255,180,.08)' }}>
              {[
                [t('pv_stat_poi'), provider.poi],
                [t('pv_stat_rate'), provider.rate],
                [t('pv_stat_jobs'), provider.jobs],
                [t('pv_stat_status'), t('pv_status_active')],
              ].map(([key, value]) => (
                <div key={`${provider.name}-${key}`}>
                  <div style={{ fontFamily:'Share Tech Mono', fontSize:'0.88rem', color: key === t('pv_stat_status') ? '#00c8ff' : '#00ffb4' }}>{value}</div>
                  <div style={{ fontSize:'0.68rem', color:'#3a4560', textTransform:'uppercase', letterSpacing:'0.06em', marginTop:2 }}>{key}</div>
                </div>
              ))}
            </div>
          </Card>
